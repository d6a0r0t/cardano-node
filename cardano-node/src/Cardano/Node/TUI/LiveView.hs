...
import           Cardano.Tracing.Peer (LVPeer (..), ppPeer)
...
data LiveViewBackend blk a = LiveViewBackend { getbe :: LiveViewMVar blk a }
...
storePeersInLiveView :: NFData a => [LVPeer blk] -> LiveViewBackend blk a -> IO ()
storePeersInLiveView peers lvbe = modifyMVar_ (getbe lvbe) $ \lvs -> return $ lvs { lvsPeers = peers }

storeMempoolDataInLiveView :: NFData a => Word64 -> Word64 -> LiveViewBackend blk a -> IO ()
storeMempoolDataInLiveView capacity capacityBytes lvbe = modifyMVar_ (getbe lvbe) $ \lvs ->
  return $ lvs { lvsMempoolCapacity = capacity
